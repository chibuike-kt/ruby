...
func newTestProcessorWithReminders(pool *pgxpool.Pool, rdb *redis.Client, extractor ai.Extractor, phraser ai.Phraser, sender ai.Sender, templateSender reminder.TemplateSender) *ai.Processor {
	return ai.NewProcessor(ai.Config{
		...
		Reminders:   reminder.NewService(pool, templateSender, "debt_reminder"),
	})
}
...
func TestProcessor_ReminderOptIn_No_NoFurtherAction(t *testing.T) {
	...
	if got := countRows(t, pool, `SELECT count(*) FROM reminders`); got != 0 {
		t.Fatalf("got %d reminders scheduled after declining, want 0", got)
	}
}
...
func TestProcessor_ReminderOptIn_Yes_PhoneAlreadyOnFile_Schedules(t *testing.T) {
	...
	if got := countRows(t, pool, `SELECT count(*) FROM reminders`); got != 2 {
		t.Fatalf("got %d reminders scheduled, want 2 (day-before and due-date)", got)
	}
}
...
func TestProcessor_ReminderOptIn_Yes_NoPhone_AsksThenSchedules(t *testing.T) {
	...
	if got := countRows(t, pool, `SELECT count(*) FROM reminders`); got != 0 {
		t.Fatalf("got %d reminders scheduled before the phone is captured, want 0", got)
	}
	...
	if got := countRows(t, pool, `SELECT count(*) FROM reminders`); got != 2 {
		t.Fatalf("got %d reminders scheduled, want 2", got)
	}
}
...
func TestProcessor_ReminderDispatch_UsesTemplatedShape(t *testing.T) {
	...
	svc := reminder.NewService(pool, templateSender, "debt_reminder")
	...
	for _, call := range templateSender.calls {
		if call.to != phone {
			t.Fatalf("got recipient %q, want the customer's phone %q — reminders go to the customer, never the trader", call.to, phone)
		}
		if call.templateName != "debt_reminder" {
			t.Fatalf("got template %q, want debt_reminder", call.templateName)
		}
